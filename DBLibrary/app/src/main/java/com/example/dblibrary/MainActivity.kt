...
class MainActivity : AppCompatActivity() {
    ...
    private fun setListener() {
        ...
        val ed_year = findViewById<EditText>(R.id.ed_year)
        findViewById<Button>(R.id.btn_insert).setOnClickListener {
            //判斷是否有填入書名或價格
            if (ed_book.length() < 1 || ed_price.length() < 1 || ed_year.length() < 1 )
                showToast("欄位請勿留空")
            else
                try {
                    //新增一筆書籍紀錄於 myTable 資料表
                    dbrw.execSQL(
                        "INSERT INTO myTable(book, price, year) VALUES(?,?,?)",
                        arrayOf(ed_book.text.toString(),
                            ed_price.text.toString(),
                            ed_year.text.toString()),
                    )
                    showToast("新增:${ed_book.text},價格:${ed_price.text},年份:${ed_year.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("新增失敗:$e")
                }
        ...
        findViewById<Button>(R.id.btn_update).setOnClickListener {
            ...
            if (ed_book.length() < 1 || ed_price.length() < 1 || ed_year.length() < 1 )
                ...
                try {
                    ...
                    dbrw.execSQL("UPDATE myTable SET price = ${ed_price.text}, year = ${ed_year.text} WHERE book LIKE '${ed_book.text}'")
                    showToast("更新:${ed_book.text},價格:${ed_price.text},年份:${ed_year.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("更新失敗:$e")
                }
        ...
        findViewById<Button>(R.id.btn_query).setOnClickListener {
            ...
            for (i in 0 until c.count) {
                //加入新資料
                items.add("書名:${c.getString(0)}\t\t\t\t 價格:${c.getInt(1)}\t\t\t\t  年份:${c.getInt(2)}")
                c.moveToNext() //移動到下一筆
            }
    ...
    private fun cleanEditText() {
        ...
        findViewById<EditText>(R.id.ed_year).setText("")
    }
}